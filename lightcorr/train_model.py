...
from config_utlis import (
    config_checks, load_config, initialize_model,
)
...
def main():
    ...
    parser.add_argument(
        '-r', '--run_name', type=str, 
        help=(
            'Name of the run followed by date time. \
    ...
    print("Single model training config:", 
          config['single_model_training_config'])
    ...
    if hyperparameter_search_type == 'grid_search':
        ...
        best_model, best_hyperparameters, cv_results = train_classifier_gridSearch(
            model, flattened_flow_pairs_train, flattened_labels_train, 
            parameter_grid, **grid_search_config)
        export_dataframe_to_csv(
            pandas.DataFrame(cv_results), 
            'grid_search_cv_results.csv', 
            run_folder_path)
        ...
        best_model, best_hyperparameters, cv_results = train_classifier_halvingGridSearch(
            model, flattened_flow_pairs_train, flattened_labels_train, 
            parameter_grid, **halving_grid_search_config)
        export_dataframe_to_csv(
            pandas.DataFrame(cv_results),
            'halving_grid_search_cv_results.csv', 
            run_folder_path)
        ...
        best_model, best_hyperparameters, cv_results = train_classifier_randomSearch(
            model, flattened_flow_pairs_train, flattened_labels_train, 
            parameter_grid, **random_search_config)
        export_dataframe_to_csv(
            pandas.DataFrame(cv_results), 
            'random_search_cv_results.csv', 
            run_folder_path)

    elif hyperparameter_search_type == 'none':
        # This is just a model, not the best model :)
        best_model = train_model(model, flattened_flow_pairs_train, flattened_labels_train)
    ...
    if config['validation_settings']['run_validation']:
        ...
        custom_cross_validate(
            best_model, flattened_flow_pairs_train, flattened_labels_train,
            roc_plot_enabled, run_folder_path, **validation_config)
