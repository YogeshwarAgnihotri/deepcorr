...
from utlis import check_path_throw_error
...
def load_dataset(path_dataset, load_only_min_300_flows=True):
    ...
    all_runs = {'8872': '192.168.122.117', '8802': '192.168.122.117',
    ...
    for name in all_runs:
        if load_only_min_300_flows:
            paths_to_pickle_files_to_load.append(
                path_dataset + '/%s_tordata300.pickle' % name)
        else:
            paths_to_pickle_files_to_load.append(
                path_dataset + '/%s_tordata.pickle' % name)
            paths_to_pickle_files_to_load.append(
                path_dataset + '/%s_tordata300.pickle' % name)
            paths_to_pickle_files_to_load.append(
                path_dataset + '/%s_tordata400.pickle' % name)
            paths_to_pickle_files_to_load.append(
                path_dataset + '/%s_tordata500.pickle' % name)
